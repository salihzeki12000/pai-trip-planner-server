 ...
 */
public class CarRouteHTTPRequest {
    private static Logger logger = Logger.getLogger(CarRouteHTTPRequest.class);
//    http://maps.google.com/maps/api/directions/json?origin=37.631886,127.0622051&destination=37.556173,127.048483&sensor=false
    ...
    public static NaverCarRoute naverCarRouteRequest(BasicPOI src, BasicPOI dest){
        String srcLongitude = Double.toString(src.getLocation().longitude);
        String srcLatitude = Double.toString(src.getLocation().latitude);
        String destLongitude = Double.toString(dest.getLocation().longitude);
        String destLatitude = Double.toString(dest.getLocation().latitude);
        String str = "http://map.naver.com/findroute2/findCarRoute.nhn?via=&call=route2&output=json&car=0&mileage=12.4&start=" + srcLongitude + "%2C"  + srcLatitude +"%2C%20&destination=" + destLongitude + "%2C"+ destLatitude + "%2C%20&search=2";
      //  String str = "http://map.naver.com/findroute2/findCarRoute.nhn?&output=json&start=" + srcLongitude + "%2C" + srcLatitude + "%2C%20&destination=" + destLongitude + "%2C" + destLatitude + "%2C%20";
        try {
            String retStr =requestHTTP(str);
            logger.debug(retStr);
            Gson gson = new Gson();
            NaverCarRoute naverCarRoute = gson.fromJson(retStr,NaverCarRoute.class);
            return naverCarRoute;
        } catch (IOException e) {
...
//        127.0622051%2C37.556173%2C%20&destination=127.048483%2C37.556173
        BasicPOI src= POIManager.getInstance().getPOIByTitle("제주국제공항");
        BasicPOI dest = POIManager.getInstance().getPOIByTitle("제주신라호텔");

        logger.debug(CarRouteHTTPRequest.naverCarRouteRequest(src,dest));
